...
void GPIO_Init(void)
{
    ...
    // PB3 
    MODIFY_REG(GPIOB->MODER, GPIO_MODER_MODER3, GPIO_MODER_MODER3_1); // AF mode
...
void TIM_Init(void)
{
    ...
    TIM1->PSC = 179; // Аналогично 1 MHz
    TIM1->ARR = 1000; // 1 kHz PWM
    
    // PWM Mode 1 для CH1, CH2, CH3, CH4
